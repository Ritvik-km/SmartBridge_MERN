...
module.exports.getMovieShows = async (req, res, next) => {
  try {
    ...
    if (!getMovie || !Array.isArray(getMovie.shows)) {
      return res.json({ status: true, showData: [] }); // Safe fallback
    }
      ...
      movieShows.map(async (s) => {
        ...
        if (dateObject > new Date()) {
          ...
          return {
            ...
            theatreName: upperCaseTheatreName,
            showId: show.showId,
          };
        }
    ...
    const filteredShowsData = showData.filter(Boolean); // remove null/undefined
...
module.exports.deleteShow = async (req, res) => {
  try {
    ...
    // console.log("prevMovieShows", prevMovieShows);
